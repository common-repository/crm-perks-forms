...
return $this->show_form( intval($atts['id']),"");
//return $html;
}
...
foreach($fields_arr as $k=>$v){ 
     ...
     $v['default']=cfx_form::clean_text($_COOKIE[$v['par_name']]);       
      ...
      if(strpos($v['default'],'{') === 0 ){
          $v['default']=trim($v['default'],'{}');
          if(strpos($v['default'],'user_') === 0 ){
              if($v['default'] == 'user_id'){ $v['default']='ID'; }
          $user=wp_get_current_user(); 
       if(is_object($user) && isset($user->{$v['default']}) ){
           $v['default']=$user->{$v['default']}; 
       }else{
           $v['default']='';
       }  
          }
...
$form_id=intval(cfx_form::post('form_id'));
$return='yes';
   ...
   return $response;     
    ...
    echo json_encode($response);
      die();
         ...
         if( isset($options['logged_in']) && $options['logged_in'] != ''){
             $user_id=get_current_user_id(); 
             if(empty($user_id)){
    $response=array("status"=>"error","msg"=>"Please login before submitting form");  
    $response=$this->res_out($response,$fixed,$entry_id,$form);
        if($return == "yes"){
   return $response;     
    }else{ 
...
if(!in_array( strtolower($ext),$allowed_files) || in_array( strtolower($ext),$not_allowed_files)){
